...
function Header({ currentUserId, account, onUserChange, onSignOut, onMenu, loading }: { currentUserId: string; account?: Profile; onUserChange: (id: string) => void; onSignOut?: () => void; onMenu: () => void; loading: boolean }) {
  const current = account ?? profiles.find((profile) => profile.id === currentUserId) ?? profiles[0]
  return <header className="topbar"><button className="menu-button" onClick={onMenu} aria-label="Open navigation"><Menu /></button><div className="catalog-health"><span className={loading ? 'catalog-dot loading' : 'catalog-dot'} /><strong>{loading ? 'Loading page' : 'Catalog online'}</strong><span>TMDB · Open Library · MusicBrainz</span></div>{account ? <div className="profile-switcher authenticated-profile"><Avatar profile={current} size="small" /><span>{current.name}</span><button onClick={onSignOut} aria-label="Sign out" title="Sign out"><LogOut size={15} /></button></div> : <label className="profile-switcher"><span className="sr-only">Profile</span><Avatar profile={current} size="small" /><select value={currentUserId} onChange={(event) => onUserChange(event.target.value)}>{profiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.name}</option>)}</select><ChevronDown size={14} /></label>}</header>
}
...
export function App({ account, onSignOut }: { account?: Profile; onSignOut?: () => void } = {}) {
